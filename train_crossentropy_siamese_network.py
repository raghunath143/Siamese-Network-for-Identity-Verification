...
if not config.SAVED_DATA:
    ...
    images_data = np.zeros((size, *config.BCEL_IMG_SHAPE), dtype='uint8')
    ...
    for k in range(size):
        ...
        image_data = cv2.resize(image_data, config.BCEL_IMG_SHAPE[::-1][1:])
...
with mirrored_strategy.scope():
    imgA = Input(shape=config.BCEL_IMG_SHAPE)
    imgB = Input(shape=config.BCEL_IMG_SHAPE)
    featureExtractor = build_siamese_model(config.BCEL_IMG_SHAPE)
    featsA = featureExtractor(imgA)
    featsB = featureExtractor(imgB)

    # finally, construct the siamese network
    distance = Lambda(utils.euclidean_distance, name="Euclidean")([featsA, featsB])
    outputs = Dense(1, activation="sigmoid", name="DenseSigmoid")(distance)
...
batch_size = config.BCEL_BATCH_SIZE
epochs = config.BCEL_EPOCHS
...
model.save(config.BCEL_MODEL_PATH)
# plot the training history
print("[INFO] plotting training history...")
utils.plot_training(history, config.BCEL_PLOT_PATH)
