...
model = load_model(config.TL_MODEL_PATH, compile=False)
...
for (i, (pathA, pathB)) in enumerate(pairs):
    ...
    preds = model.predict([imageA, imageB, imageB])
    # emb_size = 64
    # first_image_emb = preds[:, :emb_size]
    # second_image_emb = preds[:, emb_size:2 * emb_size]
    # third_image_emb = preds[:, 2 * emb_size:]
    #
    # preds = utils.euclidean_distance((first_image_emb, second_image_emb))
    proba = preds[0][0]
